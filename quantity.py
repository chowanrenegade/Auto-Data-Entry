...
def run_update():
    ...
    try:
        ...
        # Identify Website columns
        web_sku = next((c for c in df1.columns if c.lower() == 'sku'), None)
        web_stock_col = next((c for c in df1.columns if c.lower() == 'total stock'), None)

        # Identify Everest columns
        ev_code = next((c for c in df2.columns if c.lower() == 'code'), None)
        ev_stock_col = next((c for c in df2.columns if c.lower() == 'total stock'), None)
        ...
        if not ev_code:
            messagebox.showerror("Error", "Could not find 'Code' in Everest file.")
            return

        if not web_stock_col:
            messagebox.showerror("Error", "Could not find 'Total Stock' in Website file.")
            return

        if not ev_stock_col:
            messagebox.showerror("Error", "Could not find 'Total Stock' in Everest file.")
            return

        # Prepare Everest Data (SKU + Total Stock only)
        df2_prepared = df2[[ev_code, ev_stock_col]].copy()
        df2_prepared.columns = ['sku_join', 'stock_new']
        ...
        # Update Total Stock
        updated_df[web_stock_col] = updated_df['stock_new'].combine_first(updated_df[web_stock_col])

        # Cleanup internal join columns
        updated_df.drop(columns=['sku_join', 'stock_new'], inplace=True)

        # Save result
        save_path = filedialog.asksaveasfilename(
            defaultextension=".csv",
            filetypes=[("CSV file", "*.csv"), ("Excel file", "*.xlsx")],
            initialfile="Updated_Inventory_Stock.csv"
        )
        ...
        if save_path:
            ...
            messagebox.showinfo("Success", "Total Stock synced successfully!")
...
# --- GUI ---
root = tk.Tk()
root.title("Inventory Sync: Total Stock Only")
...
tk.Label(root, text="Everest File (Source of Total Stock)", font=('Arial', 10, 'bold')).pack(pady=(20,0))
...
tk.Button(root, text="SYNC TOTAL STOCK", command=run_update, 
