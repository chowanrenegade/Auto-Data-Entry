...
def run_update():
    ...
    try:
        ...
        if not web_sku:
            ...
            return
        if not all([ev_code, ev_cat, ev_desc]):
            messagebox.showerror("Error", f"Everest file needs 'Code', 'Category', and 'Description'.\nFound: {list(df2.columns)}")
            return

        # 3. Prepare Everest Data
        # We grab Code, Category, and Description
        df2_prepared = df2[[ev_code, ev_cat, ev_desc]].copy()
        df2_prepared.columns = ['sku_join', 'brand_new', 'desc_new']
        ...
        if web_brand_col in updated_df.columns:
            updated_df[web_brand_col] = updated_df['brand_new'].combine_first(updated_df[web_brand_col])
        else:
            updated_df.rename(columns={'brand_new': web_brand_col}, inplace=True)

        # 6. Handle Description Update (Name/Short Description)
        if web_desc_col in updated_df.columns:
            # Overwrite or fill the existing description column
            updated_df[web_desc_col] = updated_df['desc_new'].combine_first(updated_df[web_desc_col])
        else:
            # Create the column if it didn't exist
            updated_df.rename(columns={'desc_new': web_desc_col}, inplace=True)

        # Cleanup
        cols_to_drop = ['sku_join', 'brand_new', 'desc_new']
        ...
        save_path = filedialog.asksaveasfilename(
            ...
            initialfile="Updated_Website_Data.csv"
        )
        ...
        if save_path:
            ...
            messagebox.showinfo("Success", "Successfully updated Brands and Descriptions!")
...
# --- GUI (Same as before) ---
root = tk.Tk()
root.title("Website & Everest Linker")
...
tk.Button(root, text="UPDATE & SAVE NEW FILE", command=run_update, 
          bg="#27ae60", fg="white", font=('Arial', 12, 'bold'), height=2).pack(pady=30)
